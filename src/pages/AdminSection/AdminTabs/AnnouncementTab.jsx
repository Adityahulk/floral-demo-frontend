  ...
  return (
    <div className="max-w-2xl space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 style={{ fontFamily:"Georgia, serif", color:"var(--color-charcoal)" }} className="text-xl font-bold">Announcement Bar</h2>
          <p style={{ color:"var(--color-olive)" }} className="text-sm">{activeCount} active message{activeCount !== 1 ? "s" : ""} rotating</p>
        </div>
      ...
      <div className="flex items-center justify-between p-4 rounded-2xl border bg-white" style={{ borderColor:"var(--color-border)" }}>
        <div>
          <p style={{ color:"var(--color-charcoal)" }} className="font-semibold text-sm">Show Announcement Bar</p>
          <p style={{ color:"var(--color-olive)" }} className="text-xs mt-0.5">Hide the bar sitewide without deleting messages</p>
        </div>
      ...
      <div className="p-5 rounded-2xl border bg-white space-y-4" style={{ borderColor:"var(--color-border)" }}>
        <p style={{ color:"var(--color-charcoal)" }} className="font-semibold text-sm">Appearance</p>
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="text-xs font-medium block mb-1.5" style={{ color:"var(--color-olive)" }}>Background Color</label>
          ...
          <div>
            <label className="text-xs font-medium block mb-1.5" style={{ color:"var(--color-olive)" }}>Text Color</label>
            <div className="flex items-center gap-2">
        ...
        <div>
          <label className="text-xs font-medium block mb-1.5" style={{ color:"var(--color-olive)" }}>Rotation Interval (seconds)</label>
        ...
        {/* Live Preview */}
        <div>
          <label className="text-xs font-medium block mb-1.5" style={{ color:"var(--color-olive)" }}>Preview</label>
      ...
      <div className="p-5 rounded-2xl border bg-white space-y-3" style={{ borderColor:"var(--color-border)" }}>
        <div className="flex items-center justify-between mb-1">
          <p style={{ color:"var(--color-charcoal)" }} className="font-semibold text-sm">Messages</p>
        ...
        {(config.messages ?? []).length === 0 && (
          <p className="text-sm text-center py-4" style={{ color:"var(--color-olive)" }}>No messages yet. Add one above.</p>
        )}

        {(config.messages ?? []).map((m, i) => (
          <div key={i} className="flex items-center gap-3 p-3 rounded-xl border"
            style={{ borderColor: m.active ? "var(--color-border)" : "var(--color-border)", background: m.active ? "white" : "var(--color-beige)" }}>
            ...
            <button onClick={() => updateMsg(i, "active", !m.active)} className="shrink-0">
              {m.active
                ? <ToggleRight size={24} style={{ color:"var(--color-olive)" }} />
