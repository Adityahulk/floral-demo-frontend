  ...
  return (
    <div onMouseEnter={()=>setHovered(true)} onMouseLeave={()=>setHovered(false)}
      ...
      <div className="relative overflow-hidden" style={{aspectRatio:"4/5"}}>
        <img src={p.img} alt={p.name} className="w-full h-full object-cover transition-transform duration-500" style={{transform:hovered?"scale(1.06)":"scale(1)"}}/>
        {p.badge&&<span className={`absolute top-3 left-3 text-xs font-bold px-2 py-0.5 rounded-full ${BADGE_STYLES[p.badge]}`}>{p.badge==="Sale"?`-${disc}%`:p.badge}</span>}
      ...
      <div className="p-4">
        ...
        <div className="flex items-center gap-1.5 mb-3"><Stars n={p.rating} size={12}/><span style={{color:"#9c7a62"}} className="text-xs">({p.reviews})</span></div>
        <div className="flex items-center justify-between">
          <div className="flex items-baseline gap-1.5">
            <span style={{color:"#c97d5b"}} className="font-bold">{fmt(p.price)}</span>
            {p.original&&<span className="text-xs text-stone-400 line-through">{fmt(p.original)}</span>}
          </div>
